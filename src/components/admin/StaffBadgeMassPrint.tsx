...
import { StaffBadgeCard } from "@/components/admin/StaffBadgeCard"
import { Button } from "@/components/ui/button"

interface StaffBadgeMassPrintProps {
  users: User[]
  onClose: () => void
}

export function StaffBadgeMassPrint({ users, onClose }: StaffBadgeMassPrintProps) {
    ...
    <div className="min-w-0 space-y-4">
      <div className="badge-print-controls no-print flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-silver/60 bg-white p-3 shadow-sm sm:p-4">
        <div className="min-w-0">
          <h2 className="text-lg font-bold text-primary sm:text-xl">Print staff badges</h2>
      ...
      {sorted.length === 0 ? (
        <p className="no-print rounded-2xl border border-warning/40 bg-warning/5 p-4 text-sm text-primary">
          No staff selected. Go back and choose people to print.
        </p>
      ) : (
        <div
          ref={printRef}
        ...
        >
          <div className="badge-print-grid mx-auto grid max-w-[8.5in] grid-cols-1 justify-items-center gap-4 sm:grid-cols-2 print:max-w-none print:gap-3">
            {sorted.map((user) => (
              <StaffBadgeCard key={user.id} user={user} />
            ))}
          </div>
        </div>
      )}

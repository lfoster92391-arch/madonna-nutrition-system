...
export function DesignToolbar({
  ...
  return (
    <header className="shrink-0 border-b border-silver bg-primary text-white">
      <div className="flex max-w-full flex-wrap items-center gap-x-2 gap-y-2 overflow-x-auto px-2 py-2 [scrollbar-width:thin] sm:gap-x-3 sm:px-3 md:px-4">
        <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
          <Button
            variant="ghost"
            size="sm"
            className="min-h-9 min-w-9 text-white hover:bg-white/10 hover:text-white"
            onClick={onUndo}
            disabled={!canUndo}
            title="Undo"
          >
          ...
          {VIEWPORTS.filter(({ mode }) => !compact || mode === "mobile" || mode === "print").map(
            ({ mode, label, icon: Icon }) => (
              <button
                ...
                className={cn(
                  "flex min-h-9 min-w-9 items-center justify-center gap-1.5 rounded-lg px-2 text-xs font-semibold transition sm:px-2.5",
                  viewport === mode
        ...
        <div className="ml-auto flex shrink-0 flex-wrap items-center justify-end gap-1">
          {!compact ? (
            <Button
              variant="ghost"
              size="sm"
              className="hidden min-h-9 text-white hover:bg-white/10 hover:text-white xl:inline-flex"
              onClick={onVersionHistory}
            >
              <History className="h-4 w-4" />
              <span className="hidden 2xl:inline">Version History</span>
            </Button>
          ) : null}
          <Button
            variant="ghost"
          ...
          >
            ...
            <span className={compact ? "sr-only" : "hidden lg:inline"}>Export</span>
          </Button>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            ...
            <span className={compact ? "sr-only" : "hidden lg:inline"}>Save</span>
          </Button>
          {!compact ? (
            <Button
              variant="secondary"
              size="sm"
              className="hidden min-h-9 shrink-0 bg-white/15 text-white hover:bg-white/25 lg:inline-flex"
              onClick={onPreview}
            >
